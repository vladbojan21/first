...
var HTMLReport = function() {
  ...
  var generateSummaries = function(reportXml, report) {
    ...
    for (var i=0; i<totalSuites; i++) {
      ...
      //
      suite.duration = parseFloat(Math.ceil(testSuites[i].attr.time));
      ...
      //store suite data
      suitesData.push({'keyword': 'TestSuite', 'name': suite.name, 'duration': suite.duration, 'testcases': testCasesNames, 'testcasesresults':testCasesResults, 'testcasestimes': testCasesTimes, 'testcasesmessages': testCasesMessages, 'screenshotsNames':screenshotsNamesOnFailure, 'tests': suite.tests, 'failed': suite.failed, 'errors': suite.errors, 'skipped': suite.skipped, 'passed': suite.passed});
